...
.then(snapshot => {
  ...
  let total = 0;
  let registrationKeysWithWaive = Object.keys(registrations).filter(uid => has(registrations[uid], 'order.waiveDiscount'));
  sortBy(registrationKeysWithWaive, uid => registrations[uid].order.created_at).forEach(uid => {
    const user = users[uid];
    console.log(`${user.profile.first_name} ${user.profile.last_name}`);
    total++;
  });
